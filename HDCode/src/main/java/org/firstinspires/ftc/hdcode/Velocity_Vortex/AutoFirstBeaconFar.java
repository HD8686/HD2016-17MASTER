 ...
 */
public class AutoFirstBeaconFar implements HDAuto{
    ...
    public AutoFirstBeaconFar(double delay, Alliance alliance){
    ...
    public void runLoop(final double elapsedTime) {
        Log.w("comeBackToFirstBeacon: ", String.valueOf(comeBackToFirstBeacon));
        Log.w("State", String.valueOf(SM.getState()));
        if(SM.ready()){
            ...
            switch (states){
                ...
                case fastDriveToBeacon:
                    ...
                    robot.driveHandler.mecanumDrive_Polar_keepFrontPos(0.225, 59.0, -90.0, robot.navX.getYaw());
                ...
                case waitCheckBeacon:
                        SM.setNextState(State.backUp, HDWaitTypes.Timer, (4.5 - (elapsedTime - (var - 0.15))) + 0.5);
                        robot.shooter.setFlywheelPower(0.345);
                        if (elapsedTime < var) {
                ...
                case firstBeaconFailsafe:
                    SM.setNextState(State.wait, HDWaitTypes.Timer, 0.5);
                    robot.shooter.setCollectorPower(0.0);
                    robot.shooter.setAcceleratorPower(0.0);
                    break;
                case turnToCorner:
                    SM.setNextState(State.wait6, HDWaitTypes.Timer, 2.5);
                    ...
                    break;
                case wait6:
                    SM.setNextState(State.driveToCorner, HDWaitTypes.Timer, .05);
                    robot.driveHandler.motorBrake();
                    robot.driveHandler.resetEncoders();
                    break;
                case driveToCorner:
                    if(robot.driveHandler.getEncoderCount() < -600){
                        SM.resetValues();
                        robot.driveHandler.motorBrake();
