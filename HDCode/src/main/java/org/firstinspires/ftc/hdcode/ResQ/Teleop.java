...
public class Teleop extends OpMode implements SensorEventListener {
    ...
    public void init() {
        ...
        //Configure Display
        dashboard = new HDDashboard_2015(telemetry);
    ...
    public void TelemetrySub(){
        dashboard.displayPrintf(0, "Teleop");
        dashboard.displayPrintf(2,"TeleOp Started At : %s", startDate);
        dashboard.displayPrintf(3,"Current Run Time : %.2f", runtime.time());
        dashboard.displayPrintf(4,"Current WinchLock Status : %b", WinchLockUp);
        dashboard.displayPrintf(5,"Current WinchEnc Status : %d", WinchMotor.getCurrentPosition());
        dashboard.displayPrintf(6, "Current Roll: %d", Math.round(Math.toDegrees(roll)));
        dashboard.displayPrintf(7, "Current Tilt Mode: %s, Current Winch Pos: %f", TiltMode,CurrPosition);
        dashboard.displayPrintf(8,"Current WinchMin : %f", WinchMin);
        if (HoldActive == 1) {
            dashboard.displayPrintf(1, "Current Drive Speed: HOLDING FORCE");
        }
        else if(Speed == 2) {
            dashboard.displayPrintf(1, "Current Drive Speed: 50%%");
        }
        else {
            dashboard.displayPrintf(1, "Current Drive Speed: 100%% ");
        }
